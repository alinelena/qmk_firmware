...
void show_icons_lcd(void){
        ...
        draw_lcd_icon(40, h, last_led_state.num_lock, 3, HSV_YELLOW, HSV_BLACK);
...
void render_lcd_logo(void) {
    if (update_lcd_logo) {
       ...
       // }
        ...
        qp_drawtext_recolor(lcd, DISP_WIDTH - hash_width - 15, DISP_HEIGHT - 1.5 * h, qp_fonts[0], hash, HSV_ORANGE, HSV_BLACK);
        int16_t build_width = qp_textwidth(qp_fonts[0], bdate);
        qp_drawtext_recolor(lcd, DISP_WIDTH - build_width - 15, 10, qp_fonts[0], bdate, HSV_ORANGE, HSV_BLACK);
        }else{
        int16_t hash_width = qp_textwidth(qp_fonts[0], hash);
        qp_drawtext_recolor(lcd, DISP_WIDTH - hash_width - 20, 20, qp_fonts[0], hash, HSV_ORANGE, HSV_BLACK);
