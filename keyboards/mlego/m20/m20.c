...
const rgblight_segment_t PROGMEM my_lwr_layer[]    = RGBLIGHT_LAYER_SEGMENTS({0, RGBLIGHT_LED_COUNT, HSV_GREEN});
const rgblight_segment_t PROGMEM my_rse_layer[]    = RGBLIGHT_LAYER_SEGMENTS({0, RGBLIGHT_LED_COUNT, HSV_RED});
const rgblight_segment_t PROGMEM my_adj_layer[]    = RGBLIGHT_LAYER_SEGMENTS({0, RGBLIGHT_LED_COUNT, HSV_CORAL});
...
#if defined(RGB_MATRIX_ENABLE)
    if (toggle_rse && layer_state_is(_RSE)) {
       toggle_rgb(RGB_MATRIX_RSE_I,toggle_rse,HSV_BLUE);
    }
    if (toggle_lwr && layer_state_is(_LWR)) {
    toggle_rgb(RGB_MATRIX_LWR_I,toggle_lwr,HSV_RED);
    }
    if (layer_state_is(_ADJ) && (toggle_lwr || toggle_rse) ) {
      toggle_rgb(RGB_MATRIX_RSE_I,toggle_lwr&& toggle_rse,HSV_PINK);
      toggle_rgb(RGB_MATRIX_LWR_I,toggle_rse&&toggle_lwr,HSV_PINK);
    }
#endif
    led_lwr(toggle_lwr);
    led_rse(toggle_rse);
...
bool rgb_matrix_indicators_advanced_kb(uint8_t led_min, uint8_t led_max) {
    ...
    /* deal with capslock, use it only when has something to indicate*/
